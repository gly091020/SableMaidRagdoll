    ...
    public static final class Entry {
        ...
        private Entry(@Nullable BedrockModel<Mob> model, @Nullable MaidModelInfo info) {
            ...
            if (model == null) {
                ...
                this.ignoreParts = TLMMaidModelHelper.IGNORE_PART.stream()
        ...
        /**
         * 重置模型到初始状态，等价于 TLMMaidModelHelper.resetModel(MaidDollDefaultModel)。
         */
        public void reset() {
            for (BedrockPart part : allParts) {
                TLMMaidModelHelper.resetModel(part);
            }
            for (BedrockPart part : ignoreParts) {
                TLMMaidModelHelper.hidePart(part);
            }
        }
