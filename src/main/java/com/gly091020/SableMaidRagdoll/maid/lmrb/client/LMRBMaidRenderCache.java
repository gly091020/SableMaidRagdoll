 ...
 * <p>
 * {@link MaidDollData} 的约定：{@code modelID} 为 LMML 模型名，{@code soundID} 为贴图包名。
 */
@OnlyIn(Dist.CLIENT)
public final class LMRBMaidRenderCache {
    ...
    private static void apply(LittleMaidEntity maid, MaidDollData data) {
        TextureHolder holder = resolveHolder(data);
        if (holder != null) {
            boolean contract = true;
            maid.setContractMM(contract);
            maid.setColorMM(pickColor(holder, contract));
    ...
    private static @Nullable TextureHolder resolveHolder(MaidDollData data) {
        LMTextureManager manager = LMTextureManager.INSTANCE;
        if (!data.soundID().isEmpty()) {
            TextureHolder holder = manager.getTexture(data.soundID()).orElse(null);
            if (holder != null)
                return holder;
        }
        for (TextureHolder holder : manager.getAllTextures()) {
            if (holder.getModelName().equalsIgnoreCase(data.modelID()))
                return holder;
        }
        return null;
    }

    private static TextureColors pickColor(TextureHolder holder, boolean contract) {
